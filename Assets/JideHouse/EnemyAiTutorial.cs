...
public class EnemyAiTutorial : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        InvokeRepeating("TimerAction", 10f, 10f);
    ...
    private void TimerAction()
    {
        if (m_PhraseReady)
        {
            int[] validNumbers = { 0, 4, 7, 9 };
            int randomNumber = validNumbers[Random.Range(0, validNumbers.Length)];
            audioData.clip = audioClips[randomNumber];
            audioData.Play(0);
        }
    }
    private void Update()
    ...
    void CaughtPlayer()
    {
        if (m_PhraseReady)
        {
            ...
            audioData.Play(0);
            Destroy(gameObject.GetComponent<EnemyAiTutorial>());
            m_PhraseReady = false;
            m_CaughtPlayer = true;
    ...
    void EnviromentView()
    {
        if (!m_CaughtPlayer)
        {
            ...
            for (int i = 0; i < playerInRange.Length; i++)
            {
                ...
                if (Vector3.Angle(transform.forward, dirToPlayer) < viewAngle / 2 || Vector3.Distance(transform.position, player.position) < hearRadius)
                {
